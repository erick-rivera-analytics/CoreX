...
import { commercialClaimApplicationSchema } from "@/lib/comercial-reclamos-schemas";
...
export async function PATCH(
  ...
  try {
    ...
    const parsed = commercialClaimApplicationSchema.safeParse(raw);
    if (!parsed.success) {
      return jsonError(formatZodIssue(parsed.error.issues), 400);
    }

    const actorId = await readActorId("commercial_claim_apply");
    const data = await applyCommercialClaim(
    ...
    return handleApiError(error, "No se pudo aplicar el reclamo.");
  }
}
