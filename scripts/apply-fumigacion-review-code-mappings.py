...
OUTPUT_PATH = ROOT / "outputs" / "fumigacion_homologacion_20260525" / "fumigacion_review_code_mapping_results.json"
...
ACTOR_ID = "codex_fumigation_review_mapping"
CHANGE_REASON = "APPLY_FUMIGATION_REVIEW_CODE_MAPPING"
...
def load_review_targets() -> tuple[dict[str, set[str]], list[dict[str, object]]]:
    wb = load_workbook(WORKBOOK_PATH)
    ws = wb["Codigos por resolver"]
    headers = [cell.value for cell in ws[1]]

    code_to_activities: dict[str, set[str]] = defaultdict(set)
    ...
    for values in ws.iter_rows(min_row=2, values_only=True):
        ...
        for code in codes:
            code_to_activities[code].update(activities)

    return code_to_activities, pending_rows
...
def main() -> None:
    code_to_activities, pending_rows = load_review_targets()
    env = load_env()
    ...
    try:
        ...
        for code, required_activities in sorted(code_to_activities.items()):
            product = products_by_code.get(code)
            if not product:
                results["codes_not_found"].append({
                    "product_code": code,
            ...
            missing_activities = [activity for activity in sorted(required_activities) if activity not in current_activities]

            if not missing_activities:
                results["already_covered_codes"].append({
                    "product_code": code,
        ...
        print(json.dumps({
            "updated_codes": len(results["updated_codes"]),
            "already_covered_codes": len(results["already_covered_codes"]),
